...
class OwnSegNet(nn.Module):
    ...
    def forward(self, x):
        dim1 = x.shape
        x = F.relu(self.enc_l1_1_bn(self.enc_l1_1(x)))
        x = F.relu(self.enc_l1_2_bn(self.enc_l1_2(x)))
        
        x,idx_l1 = self.pool(x)
        
        dim2 = x.shape

        x = F.relu(self.enc_l2_1_bn(self.enc_l2_1(x)))
        x = F.relu(self.enc_l2_2_bn(self.enc_l2_2(x)))
        x,idx_l2 = self.pool(x)

        dim3 = x.shape

        x = F.relu(self.enc_l3_1_bn(self.enc_l3_1(x)))
        x = F.relu(self.enc_l3_2_bn(self.enc_l3_2(x)))
        x = F.relu(self.enc_l3_3_bn(self.enc_l3_3(x)))
        x,idx_l3 = self.pool(x)

        dim4 = x.shape

        x = F.relu(self.enc_l4_1_bn(self.enc_l4_1(x)))
        x = F.relu(self.enc_l4_2_bn(self.enc_l4_2(x)))
        x = F.relu(self.enc_l4_3_bn(self.enc_l4_3(x)))
        x,idx_l4 = self.pool(x)

        dim5 = x.shape

        x = F.relu(self.enc_l5_1_bn(self.enc_l5_1(x)))
        x = F.relu(self.enc_l5_2_bn(self.enc_l5_2(x)))
        x = F.relu(self.enc_l5_3_bn(self.enc_l5_3(x)))
        x,idx_l5 = self.pool(x)

        x = self.unpool(x,idx_l5,output_size = dim5)
        x = F.relu(self.dec_l5_1_bn(self.dec_l5_1(x)))
        x = F.relu(self.dec_l5_2_bn(self.dec_l5_2(x)))
        x = F.relu(self.dec_l5_3_bn(self.dec_l5_3(x)))

        x = self.unpool(x,idx_l4,output_size = dim4)
        x = F.relu(self.dec_l4_1_bn(self.dec_l4_1(x)))
        x = F.relu(self.dec_l4_2_bn(self.dec_l4_2(x)))
        x = F.relu(self.dec_l4_3_bn(self.dec_l4_3(x)))

        x = self.unpool(x,idx_l3,output_size = dim3)
        x = F.relu(self.dec_l3_1_bn(self.dec_l3_1(x)))
        x = F.relu(self.dec_l3_2_bn(self.dec_l3_2(x)))
        x = F.relu(self.dec_l3_3_bn(self.dec_l3_3(x)))

        x = self.unpool(x,idx_l2,output_size = dim2)
        x = F.relu(self.dec_l2_1_bn(self.dec_l2_1(x)))
        x = F.relu(self.dec_l2_2_bn(self.dec_l2_2(x)))

        x = self.unpool(x,idx_l1,output_size = dim1)
        x = F.relu(self.dec_l1_1_bn(self.dec_l1_1(x)))
        x = self.dec_l1_2(x)
        
        x_softmax = F.softmax(x, dim=1)       

        #Those need to be returned both since the CrossEntropy in pytorch
        #Does it's own softmax operation
        return x,x_softmax
